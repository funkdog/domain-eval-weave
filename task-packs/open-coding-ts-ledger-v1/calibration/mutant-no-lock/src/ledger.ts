...
export class ReservationLedger {
  ...
  async reserve(request) {
    ...
    const existing = current.reservations.find((entry) => entry.requestId === request.requestId);
    if (existing) {
      if (existing.key !== request.key || existing.units !== request.units) throw new Error("conflicting replay");
      return structuredClone(existing);
    }
    if (snapshot(current).used + request.units > current.capacity) throw new Error("capacity exceeded");
    await new Promise((resolvePromise) => setImmediate(resolvePromise));
    const reservation = { ...request, status: "pending" };
    const next = { ...current, reservations: [...current.reservations, reservation] };
    await this.#persist(next);
    this.#state = next;
    return structuredClone(reservation);
  }
  async #transition(requestId, target) {
  ...
  async snapshot() { return snapshot(this.#state); }
}
