...
function snapshot(state) {
  const reservations = structuredClone(state.reservations).sort((a, b) =>
    a.requestId.localeCompare(b.requestId) || a.key.localeCompare(b.key),
  );
  return {
    version: 1,
    capacity: state.capacity,
    used: reservations.filter((entry) => entry.status !== "released").reduce((sum, entry) => sum + entry.units, 0),
    reservations,
  };
}
...
export class ReservationLedger {
  ...
  static async open(file, capacity) {
    ...
    try {
      const state = validateState(JSON.parse(await readFile(file, "utf8")), capacity);
      return new ReservationLedger(file, state);
    } catch (error) {
  ...
  release(requestId) {
    return this.#transition(requestId, "released");
  }
