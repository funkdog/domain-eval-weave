...
function validateState(value, capacity) {
  ...
  for (const entry of value.reservations) {
    ...
    if (!["pending", "committed", "released"].includes(entry.status))
      throw new Error("invalid state");
  }
  return value;
}
...
function snapshot(state) {
  const reservations = structuredClone(state.reservations).sort(
    (a, b) => a.requestId.localeCompare(b.requestId) || a.key.localeCompare(b.key),
  );
  return {
    version: 1,
...
export class ReservationLedger {
  ...
  static async open(file, capacity) {
    ...
    try {
      return new ReservationLedger(
        file,
        validateState(JSON.parse(await readFile(file, "utf8")), capacity),
      );
    } catch (error) {
      if (error?.code !== "ENOENT") throw error;
      const ledger = new ReservationLedger(file, { version: 1, capacity, reservations: [] });
  ...
  reserve(request) {
    return this.#enqueue(async () => {
      ...
      if (existing) {
        if (existing.key !== request.key || existing.units !== request.units)
          throw new Error("conflicting replay");
        return structuredClone(existing);
      }
      if (snapshot(this.#state).used + request.units > this.#state.capacity)
  ...
  #transition(requestId, target, persist = true) {
    return this.#enqueue(async () => {
      ...
      const next = {
        ...this.#state,
        reservations: this.#state.reservations.map((entry) =>
          entry.requestId === requestId ? { ...entry, status: target } : entry,
        ),
      };
      if (persist) await this.#persist(next);
      this.#state = next;
  ...
  commit(requestId) {
    return this.#transition(requestId, "committed");
  }

  release(requestId) {
    return this.#transition(requestId, "released", false);
  }

  snapshot() {
    return this.#enqueue(async () => snapshot(this.#state));
  }
}
