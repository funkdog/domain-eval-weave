...
test("package preserves the DSH bundle and exposes the standalone Capsule surface", async () => {
  ...
  assert.equal(
    ...
    "pnpm --workspace-root build:packages && node scripts/clean-dist.mjs && tsc -p tsconfig.json && node scripts/bundle-delivery.mjs",
    "every package build must clean output and hide trusted Delivery builders in one facade bundle",
  );
});

test("legacy package snapshots the public Capsule example and repository license exactly", async () => {
  const legacyExampleRoot = fileURLToPath(
    new URL("../../examples/capsules/commerce-cancellation/", import.meta.url),
  );
  const publicExampleRoot = fileURLToPath(
    new URL("../../../../packages/weave/examples/commerce-cancellation/", import.meta.url),
  );
  assert.equal(
    await fingerprintPackageContent(legacyExampleRoot),
    await fingerprintPackageContent(publicExampleRoot),
  );
  assert.deepEqual(
...
test("clean packed artifact contains importable DSH entrypoints", async () => {
  ...
  try {
    ...
    for (const path of [
      "dist/contracts/canonical-json.js",
      "dist/contracts/canonical-json.d.ts",
