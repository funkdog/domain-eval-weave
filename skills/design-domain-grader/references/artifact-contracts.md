...
Use canonical JSON. Keep every reference relative to the pack root, normalized, and free of `..`, absolute paths, backslashes, empty segments, symlinks, and special filesystem entries.
...
- DecisionQuestion persists blocking scope and resolution/withdrawal evidence.
...
Every DomainArtifactPointer includes the exact relative ref and SHA-256 of canonical bytes. OwnerConfirmationPointer instead contains `confirmation_id + sha256` and must resolve in the protected runtime ledger. Recompute and compare before using a referenced object.
...
- Contract versions preserve stable Claim IDs, explicit predecessors, and supersede/retire transition confirmations.
...
Fail closed at the first invalid layer and preserve the invalid bytes for diagnosis.
